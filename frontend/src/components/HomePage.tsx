...
export default function HomePage() {
  const LINES = ["INTELLIGENT", "PC BUILD", "ANALYSIS"];
  ...
  useEffect(() => {
    ...
    const typeLine = () => {
      ...
      if (charIndex <= fullLine.length) {
        const newText = fullLine.slice(0, charIndex);
        ...
        const delay = 20 + Math.random() * 40 + (Math.random() < 0.1 ? 80 : 0);
    ...
    timeoutId = setTimeout(typeLine, currentLineIndex === 0 ? 400 : 0);
  ...
  return (
    <div className="relative w-full overflow-x-hidden">
      ...
      {/* ── HERO ─────────────────────────────────────────────────────── */}
      <section className="hero py-24 md:py-32 flex flex-col items-center justify-center text-center">
        <div className="container max-w-6xl mx-auto px-6 w-full flex flex-col items-center">
          <div className="hero-eyebrow mb-8 flex justify-center uppercase tracking-widest text-xs font-bold">
            V2.0 — DATASET DRIVEN
          </div>
...
=======
          <h1 className="hero-title mb-8 text-4xl md:text-6xl font-extrabold tracking-tight">
            <span className="typed-text">
              {displayLines.map((line, i) => {
                ...
                return (
                  <span key={i}>
                    {line}
                    {i === 1 && line.length > 0 && <span className="slash text-red-500"> //</span>}
                    {i < currentLineIndex && "\n"}
                  </span>
                );
              })}
            </span><span className="cursor text-red-500">_</span>
          </h1>
>>>>>>> d83ee7437ecf82bea628cdfb5194bd1def996232

          <p className="hero-sub animate-fade-in mb-12 max-w-2xl text-base md:text-lg leading-relaxed text-gray-400" style={{ animationDelay: "80ms" }}>
          ...
          <div className="hero-cta animate-fade-in flex flex-col sm:flex-row items-center justify-center gap-4 mb-16" style={{ animationDelay: "160ms" }}>
            <Link href="/builder" className="btn btn-cta btn-lg px-8 py-4">
              &gt; Start Build
            </Link>
            <a href="#features" className="btn btn-secondary btn-lg px-8 py-4">
              Explore Features
            </a>
          </div>
...
=======
          <div
            className="stats-bar animate-slide-up grid grid-cols-2 lg:grid-cols-4 gap-0 w-full rounded-xl overflow-hidden border border-white/10 bg-black/40 backdrop-blur-sm"
          ...
          >
            {[
              ...
              <div className="stat-bar-item flex flex-col items-center justify-center p-6 border-r border-b lg:border-b-0 border-white/5 last:border-r-0" key={s.label}>
                <span className="stat-bar-num text-3xl font-black text-red-500 mb-2">{s.n}</span>
                <span className="stat-bar-label text-xs tracking-widest text-gray-500 font-semibold uppercase">{s.label}</span>
              </div>
            ))}
          </div>
>>>>>>> d83ee7437ecf82bea628cdfb5194bd1def996232
      ...
      <section id="features" className="section-block">
        <div className="container">
          ...
          >
            ...
            >
=======
      <section id="features" className="py-24 md:py-32 relative z-10 bg-neutral-950/50">
        <div className="container max-w-6xl mx-auto px-6">
          <div className="text-center mb-16">
            <p className="hero-eyebrow mb-4 flex justify-center uppercase tracking-widest text-xs font-bold">
              CAPABILITIES
            </p>
...
>>>>>>> d83ee7437ecf82bea628cdfb5194bd1def996232
          ...
          <div className="features-grid stagger grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {FEATURES.map((f) => (
              <div className="feature-card animate-fade-in flex flex-col h-full bg-neutral-900 border border-white/10 rounded-xl p-8 hover:bg-neutral-800 transition-colors" key={f.title}>
                <span className="feature-tag mb-4 uppercase tracking-widest text-xs font-bold text-red-500">{f.tag}</span>
                <h3 className="feature-title text-xl font-bold mb-3">{f.title}</h3>
                <p className="feature-desc text-gray-400 text-sm leading-relaxed flex-grow">{f.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="how" className="section-block">
        <div className="container">
          <div style={{ textAlign: "center", marginBottom: 40 }}>
            ...
            >
=======
      <section id="how" className="py-24 md:py-32 relative z-10">
        <div className="container max-w-6xl mx-auto px-6">
          <div className="text-center mb-16">
            <p className="hero-eyebrow mb-4 flex justify-center uppercase tracking-widest text-xs font-bold">
              WORKFLOW
...
>>>>>>> d83ee7437ecf82bea628cdfb5194bd1def996232
          ...
          <div className="how-grid grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            {HOW_STEPS.map((s) => (
              <div className="how-step animate-slide-up flex flex-col h-full relative bg-neutral-900 border border-white/10 rounded-xl p-8 hover:border-white/20 transition-all" key={s.n}>
                <span className="how-num absolute top-6 right-6 text-6xl font-black text-white/5 pointer-events-none select-none">{s.n}</span>
                <h3 className="how-step-title text-xl font-bold mb-4 pr-12 relative z-10">{s.title}</h3>
                <p className="how-step-desc text-gray-400 text-sm leading-relaxed flex-grow relative z-10">{s.desc}</p>
              </div>
            ))}
          </div>

          <div className="text-center mt-12 md:mt-16">
            <Link href="/builder" className="btn btn-primary btn-lg inline-flex px-8 py-4">
              &gt; Launch Builder
            </Link>
          </div>
        </div>
...
=======
      {/* ── CONTACT ──────────────────────────────────────────────────── */}
      <section id="contact" className="py-24 md:py-32 relative z-10 bg-neutral-950/50">
        <div className="container max-w-6xl mx-auto px-6">
          <div className="text-center mb-16">
            <p className="hero-eyebrow mb-4 flex justify-center uppercase tracking-widest text-xs font-bold">
              CONTACTS
...
=======
          <div className="flex flex-col sm:flex-row flex-wrap justify-center items-center gap-4 md:gap-6">
            <Link
              href="mailto:gav.benedict2005@gmail.com"
              className="btn btn-primary btn-lg flex items-center justify-center gap-3 px-8 py-4 w-full sm:w-auto"
            >
              &gt; Email
      ...
      >
        <div className="container">
          <p className="term-label" style={{ justifyContent: "center", display: "flex" }}>
            ...
            by Gavin N Benedict
=======
      <footer className="py-8 md:py-12 border-t border-white/5 relative z-10 bg-black">
        <div className="container max-w-6xl mx-auto px-6 text-center flex flex-col items-center justify-center">
          <p className="term-label flex items-center justify-center gap-2 mb-3 text-xs uppercase tracking-widest text-gray-500 font-semibold">
            <span className="text-red-500">&gt;</span> PCFORGE AI v2.0 — Intelligent Build Analysis by Gavin N Benedict
>>>>>>> d83ee7437ecf82bea628cdfb5194bd1def996232
          </p>
